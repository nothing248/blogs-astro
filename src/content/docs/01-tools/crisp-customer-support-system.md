---
title: "Crisp客服系统"
filename: crisp-customer-support-system
summary: Crisp 是一款免费的在线客服系统，支持多渠道沟通。本笔记目前仅包含基本简介，详细的集成与配置方法有待完善。
tags: ["Crisp", "Customer-Support", "SaaS", "Live-Chat"]
aliases: ["Crisp客服系统", "在线对话工具"]
status: pending
date created: 星期二, 二月 25日 2025, 3:24:29 下午
date modified: 星期五, 六月 19日 2026, 11:58:21 中午
---

<!-- toc -->

## 1. 简介

**Crisp** 是一款流行的 SaaS 级在线客服系统，旨在为企业提供与客户实时沟通的桥梁。它提供了一站式的收件箱，可以集成来自网站插件、邮件、Messenger、WhatsApp 等多个渠道的咨询。

## 2. 核心功能

- **实时对话 (Live Chat)**：轻量级的网页插件。
- **知识库**：帮助客户自助解决问题。
- **CRM 集成**：管理客户信息与历史对话。

## 3. 参考资料

- [Crisp 官方网站](https://crisp.chat/en/)
