...
title: "阿里云服务器维护"
description: 本笔记整理了针对阿里云 ECS 服务器的一系列深度运维与优化措施。内容涵盖 SSH 证书快速同步方案、系统安全/监控组件（云监控、安防插件、云助手）的停用与卸载方法，旨在降低资源占用并避免不必要的系统干预。此外，详细介绍了 Journald 日志滚动策略优化、Swap 虚拟内存的手动构建与持久化挂载步骤，以及利用 MTR 进行网络链路压测的实战技巧。
