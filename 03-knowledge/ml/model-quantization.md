...
summary: 模型量化是模型部署与优化的核心技术之一，通过将权重和激活值从高精度浮点数（如 FP32）映射到低精度定点数（如 INT8）来缩减模型大小、降低内存占用并加速推理。本笔记整理了模型量化的定义、主要优势，并引向 TensorFlow Lite 官方训练后量化技术文档，为边缘端和移动端模型部署优化提供指引。
tags:
  - mlops
  - model-compression
  - quantization
