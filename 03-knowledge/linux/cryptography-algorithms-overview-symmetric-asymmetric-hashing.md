...
title: "加密算法分类"
summary: 本笔记提供了一份极简的现代密码学基础算法分类速查字典。清晰界定了三大核心密码学领域的概念边界：对称加密（AES/DES，单密钥高速加解密）、非对称加密（RSA/ECC，公私钥对用于安全交换与数字签名），以及信息摘要算法（MD5/SHA256，不可逆哈希用于校验数据完整性与密码防泄漏存储）。为后端开发中的接口验签、HTTPS 握手及敏感数据存储提供算法选型基准。
