...
description: 解析大语言模型中 Seed（种子）参数的工作原理。基于伪随机数生成器（PRNG）的特性，说明 Seed 作为初始输入值如何控制文本生成阶段的概率抽样过程，从而实现确定性输出。阐述了 Seed 在科学实验复现、Prompt 调试和软件单元测试中的应用场景，并探讨了由于 GPU 浮点计算不确定性、分布式推理和 MoE 架构等硬件/软件因素导致固定 Seed 后输出依然存在细微差异的原因。
tags:
  - 随机数种子
  - 抽样机制
  - 确定性推理
