...
description: 机器学习中常用的距离度量与相似度计算方法指南。系统解析了欧氏距离（L2 范数）、余弦相似度、曼哈顿距离（L1 范数）、切比雪夫距离（L_infinity 范数）以及汉明距离的定义、计算公式和核心特点。通过多维度对比，归纳了各度量方式在连续/离散数据类型、几何意义、异常值鲁棒性等方面的差异，为文本检索、向量嵌入比较和错误纠正提供选型依据。
tags:
  - distance-metric
  - similarity-measure
  - embedding
